...
async function setupApp() {
  ...
  app.post("/api/auth/login", async (req, res) => {
    ...
    if (!passwordMatch && user.password) return res.status(401).json({ error: "Invalid email or password" });
    // Note: if user.password is empty (e.g. from previous mock users), we still reject unless we want to let them in without password. Let's just reject.
  ...
  app.post("/api/auth/register", async (req, res) => {
    const { name, email, phone, role, permissionLevel, department, avatarUrl, password } = req.body;
